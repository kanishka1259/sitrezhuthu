...
export function DarkThemeTemplate({ data }: DarkThemeTemplateProps) {
  ...
  const s: TemplateStyles = { ...TEMPLATE_DEFAULTS['dark'], ...(data.templateStyles || {}) };
  ...
  return (
    <div style={{ minHeight: '100vh', background: s.bgColor, color: s.textColor, fontFamily: `'${bFont}', sans-serif` }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=${hFont.replace(/ /g,'+')}:wght@400;600;700&family=${bFont.replace(/ /g,'+')}:wght@400;500;600&family=JetBrains+Mono:wght@400;500&display=swap');
        ...
        .dk-skill { padding:.75rem 1rem;background:${s.cardBg};border:1px solid ${s.borderColor};border-radius:${r}px;font-size:.85rem;font-weight:500;color:${s.textColor};text-align:center;transition:all .2s;cursor:default; }
        ...
        .dk-cta { display:inline-flex;align-items:center;gap:.5rem;padding:.8rem 1.6rem;background:linear-gradient(135deg,${p},${sec});border-radius:${bR}px;font-weight:600;font-size:.9rem;color:${contrastColor(p)};text-decoration:none;cursor:pointer;border:none;transition:opacity .2s,transform .2s; }
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: `${s.bgColor}eb`, backdropFilter: 'blur(12px)', borderBottom: `1px solid ${s.borderColor}`, padding: '1rem 0' }}>
        <div style={{ maxWidth: maxW, margin: '0 auto', padding: '0 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ display: 'inline-block', fontSize: '1.1rem', fontWeight: 700, letterSpacing: '-.01em', backgroundImage: `linear-gradient(90deg,${p},${sec})`, WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent', color: 'transparent', fontFamily: `'${s.headingFont}', sans-serif` }}>
            {data.name || 'Portfolio'}
          </span>
      ...
      <div style={{ maxWidth: maxW, margin: '0 auto', padding: '6rem 2rem 4rem', display: 'grid', gridTemplateColumns: s.heroLayout === 'split' && data.avatar && s.showAvatar ? '1fr auto' : '1fr', gap: '3rem', alignItems: 'center' }}>
        <div style={{ textAlign: s.heroLayout === 'center' ? 'center' : 'left', display: 'flex', flexDirection: 'column', alignItems: s.heroLayout === 'center' ? 'center' : 'flex-start' }}>
          ...
          <h1 style={{ fontSize: 'clamp(2.5rem,6vw,4rem)', fontWeight: 700, letterSpacing: '-.03em', lineHeight: 1.08, marginBottom: '1.5rem', fontFamily: `'${s.headingFont}', sans-serif`, color: s.textColor }}>
            Hi, I&apos;m{' '}<span style={{ display: 'inline-block', backgroundImage: `linear-gradient(135deg, ${p} 0%, ${sec} 100%)`, WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent', color: 'transparent' }}>{data.name || 'Your Name'}</span>
          </h1>
          <p style={{ fontSize: '1rem', color: s.mutedColor, lineHeight: 1.75, maxWidth: s.heroLayout === 'center' ? 600 : 520, marginBottom: '2.5rem' }}>{data.bio || 'A passionate developer building elegant, scalable solutions for the modern web.'}</p>
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: s.heroLayout === 'center' ? 'center' : 'flex-start' }}>
        ...
        {data.avatar && s.showAvatar && (
          <div style={{ width: 200, height: 200, position: 'relative', flexShrink: 0, margin: s.heroLayout === 'center' ? '0 auto' : '0' }}>
            <Image 
              src={data.avatar} 
      ...
      {/* Contact */}
      <section id="contact" style={{ maxWidth: maxW, margin: '0 auto', padding: `${s.sectionGap}px 2rem`, borderTop: `1px solid ${s.borderColor}`, textAlign: 'center' }}>
        ...
        <h2 style={{ fontSize: 'clamp(1.75rem,4vw,2.75rem)', fontWeight: 700, letterSpacing: '-.02em', marginBottom: '.75rem', fontFamily: `'${s.headingFont}', sans-serif` }}>
          Let&apos;s build something{' '}<span style={{ display: 'inline-block', backgroundImage: `linear-gradient(90deg,${p},${sec})`, WebkitBackgroundClip: 'text', backgroundClip: 'text', WebkitTextFillColor: 'transparent', color: 'transparent' }}>great</span>
        </h2>
