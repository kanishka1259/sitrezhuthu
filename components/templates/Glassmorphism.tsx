...
export function GlassmorphismTemplate({ data }: GlassmorphismTemplateProps) {
  ...
  const s: TemplateStyles = data.templateStyles ?? TEMPLATE_DEFAULTS['glassmorphism'];
  ...
  return (
    <div style={{ minHeight: '100vh', background: s.bgColor, fontFamily: `'${s.bodyFont}', sans-serif`, position: 'relative', overflow: 'hidden' }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=${s.headingFont.replace(/ /g,'+')}:wght@300;400;600;700;800&family=${s.bodyFont.replace(/ /g,'+')}:wght@400;500;600&display=swap');
        ...
        .gl-card-hover:hover { background:rgba(61,170,122,.11);border-color:rgba(61,170,122,.22);transform:translateY(-4px);box-shadow:0 20px 60px rgba(0,0,0,.3); }
        ...
        .gl-nav-link:hover { background:rgba(61,170,122,.12);color:#fff; }
        .gl-skill { padding:.55rem 1.1rem;background:${s.cardBg};border:1px solid ${s.borderColor};border-radius:${r}px;font-size:.85rem;font-weight:500;color:rgba(61,170,122,.85);transition:all .2s;cursor:default; }
        .gl-skill:hover { background:${p}26;border-color:${p}66;color:${p};transform:translateY(-2px); }
        ...
        .gl-btn-ghost { display:inline-flex;align-items:center;gap:.5rem;padding:.85rem 1.75rem;background:${s.cardBg};border-radius:${bR}px;font-weight:700;font-size:.9rem;color:rgba(61,170,122,.8);text-decoration:none;border:1px solid ${s.borderColor};cursor:pointer;transition:all .2s; }
        .gl-btn-ghost:hover { background:rgba(61,170,122,.14);border-color:rgba(61,170,122,.3);transform:translateY(-2px); }
        .gl-proj-link { display:inline-flex;align-items:center;gap:.3rem;font-size:.78rem;font-weight:600;padding:.4rem .85rem;border-radius:${Math.min(r,8)}px;text-decoration:none;transition:all .2s;background:${s.cardBg};color:rgba(61,170,122,.8);border:1px solid ${s.borderColor}; }
      ...
      <div style={{ color: '#3DAA7A', position: 'relative', zIndex: 1 }}>
        {/* Nav */}
        <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(61,170,122,.05)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderBottom: `1px solid ${s.borderColor}`, padding: '1rem 0' }}>
          <div style={{ maxWidth: maxW, margin: '0 auto', padding: '0 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: '1.2rem', fontWeight: 800, letterSpacing: '-.02em', background: `linear-gradient(90deg, ${p}, ${sec})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontFamily: `'${s.headingFont}', sans-serif` }}>
              {data.name || 'Portfolio'}
            </span>
        ...
        <section id="about" style={{ maxWidth: maxW, margin: '0 auto', padding: '5rem 2rem 4rem' }}>
          ...
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '.5rem', padding: '.4rem 1rem', background: 'rgba(167,243,208,.1)', border: '1px solid rgba(167,243,208,.25)', borderRadius: '999px', fontSize: '.75rem', fontWeight: 600, color: '#6ee7b7', marginBottom: '1.5rem' }}>
              <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#3DAA7A', position: 'relative' }}>
            ...
            <h1 style={{ fontSize: 'clamp(2.2rem,5vw,3.75rem)', fontWeight: 800, lineHeight: 1.1, letterSpacing: '-.03em', marginBottom: '1.25rem', fontFamily: `'${s.headingFont}', sans-serif` }}>
              {data.name || 'Your Name'}<br />
              <span style={{ background: `linear-gradient(135deg, ${p} 0%, ${sec} 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>crafting digital</span><br />experiences
            </h1>
            <p style={{ fontSize: '1rem', color: s.mutedColor, lineHeight: 1.75, marginBottom: '2rem' }}>{data.bio || "Hi! I'm a developer who loves creating beautiful, functional digital products."}</p>
            <div className="gl-btn-group" style={{ display: 'flex', gap: '.75rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
            ...
            <div className="gl-social-group" style={{ display: 'flex', gap: '1.25rem', flexWrap: 'wrap' }}>
        ...
        <div className="gl-mobile-nav" style={{ position: 'fixed', bottom: '1.5rem', left: '50%', transform: 'translateX(-50%)', background: 'rgba(61,170,122,0.1)', backdropFilter: 'blur(20px)', padding: '.6rem 1.25rem', borderRadius: '999px', display: 'none', gap: '1.25rem', border: '1px solid rgba(61,170,122,0.2)', boxShadow: '0 10px 40px rgba(0,0,0,0.4)', zIndex: 100 }}>
